...
class Translate130mDataset(Dataset):
    def __init__(self):
        conn = _db_connect()
        cur = conn.execute("SELECT id FROM vectors_130m")
    ...
    def __getitem__(self, idx):
        ...
        conn = _db_connect()
        cur = conn.execute(
            "SELECT src_ja_anc, src_ja_pos, src_en_pos, dst_en_pos, dst_en_neg FROM vectors_130m WHERE id = ?",
            (id,),
        )
